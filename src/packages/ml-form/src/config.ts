...
export function setComponentsPreset(prefix: string, others: ComponentsPreset = {}) {
  componentsPreset = Object.assign(
    {
      input: {
        tag: prefix + 'input',
        nodeData: {
...
export function getFormColumn<D>(mlFormColumn: MlFormColumn<D>): MlFormColumn<D> {
  ...
  if (!config.placeholder && config.label) {
    config.placeholder = `${placeholderPrefix}${config.label}`
  }
  if (!config.error && config.label) {
    config.error = `${placeholderPrefix}正确的${config.label}`
  }
  // 初始化显示条件
  ...
  if (!(config.type || config.tag || config.render) || ['string'].includes(config.type)) {
