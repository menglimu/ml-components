...
export default class FormItem<D> extends Vue {
  @Prop({ required: true }) configItem!: MlFormColumn<D>
  // 原始的value值，涉及到数据格式化转换问题，通过一个值进行中转
  @Prop({ required: true }) originalValue!: any
  @Prop({ required: true }) rootValue!: any
  @Prop({ required: true }) rootConfig!: MlFormConfig<D>

  @Prop({ required: true }) tags!: Tags

  show: boolean | undefined | null = null // 是否显示隐藏
  beforeHideValue: any = null // 隐藏前的值，进行保存，显示的时候，再进行回显
  options: AnyObj[] = [] // 下拉，单选，多选等的选择项

  // isNotValue = false // 判断当前值是否为空，因为element的error框的动画原因，不实时更新，

  // 因为动态问题，改问porps传值
  // inject: ['rootValue', 'rootConfig'],

  get value(): any {
    let value = this.originalValue
    if (this.config_.format && this.config_.format.toEleValue) {
      value = this.config_.format.toEleValue(value, this.rootValue)
    }
    // 稍后判断当前值是否为空
    // setTimeout(() => {
    //   this.isNotValue = isNull(this.value)
    // }, 50)
    return value
  }
  config_: MlFormColumn<D>
  // 根据条件来显示隐藏
  get isShow(): boolean | undefined {
  ...
  @Watch('configItem', { immediate: true, deep: true })
  async onConfigChange() {
    this.config_ = getFormColumn(this.configItem)
  }

  @Watch('configItem.optionsGet', { immediate: true })
  ...
  onInput(val: any) {
    ...
    if (this.config_.format && this.config_.format.toValue) {
